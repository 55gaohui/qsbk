	...
	<view>
		<view class="body">
			<block v-for="(item,index) in list" :key="index">
				<home-list-item :item="item" :index="index" @updateuserbind="__init()"></home-list-item>
			</block>
		</view>
	</view>
</template>
		...
		methods: {
			async __init(){
				...
				this.list = [
						...
						{icon: "", name: "邮箱绑定", clicktype: "navigateTo", url: "../../pages/user-set-email/user-set-email?email="+this.User.userinfo.email, data: this.User.userinfo.email || '未绑定'},
				];
				// #ifdef APP-PLUS
				// 获取当前用户第三方登录绑定情况
				let arr = [
